# 태백과외 (taebaek-tutoring) — 작업 규칙

강원특별자치도 태백시 지역 타겟 과외 사이트. 정적 사이트, 빌드 없음.
...
## 도메인 / 배포
- 도메인: **naduedu.co.kr** (가비아, 2026-09-21 연결). `CNAME` 파일 있음 · 주소 `https://naduedu.co.kr/`
...
- 검색엔진 소유확인: 네이버 `5f6ef62ea73b2b4c1f18ff0813fcf5df1fc9f83a` · 구글 `BJ6JwBKBjbFRGqO5VTpeLXu7VPfP8vXHsJB9yGUS1P0` — 홈 `<head>` 에 생성기가 넣는다(`regions.js` 의 `verify`). **`index.html` 에 손으로 넣지 말 것** — 재생성하면 사라진다. 코드를 바꾸려면 `verify` 를 고치고 재생성.
- 링크 미리보기(카톡·네이버 공유 썸네일): `og-image.jpg` (1200x630). 전 페이지 `og:image`·`twitter:card=summary_large_image` 는 **생성기가 넣는다**(`regions.js` 의 `ogImage`). 그림을 다시 만들려면 `사이트관리/도구/지역과외/og만들기.ps1` (원본 사진 `소재/학생-여-교실.png`). 그림 파일 자체는 재생성해도 안 지워진다.
...
| `schools.html` | 태백 초·중·고 23곳 목록 + 검색 |
| `school/*.html` | **학교별 정적 페이지 23개** — SEO 의 핵심. 학교당 국어·영어·수학·사회·과학·코딩 키워드 |
| `teachers.html` | 선생님 (화상 673명 — 태백 방문 선생님이 0명이라 방문 관련 내용은 감춰져 있다) |
...
5. `& "$env:USERPROFILE\사이트관리\로그기록.ps1" -Repo taebaek-tutoring -Status 완료 -What "글 N편"`
